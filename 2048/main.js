...
Game.prototype.checkResult = function () {
  ...
  if (planFull) {
    for (var i = 0; i < this.rows; i++) {
      for (var j = 0; j < this.cols; j++) {
        if (
          (this.withinBounds(i, j + 1) &&
            this.plan[i][j] === this.plan[i][j + 1]) ||
          (this.withinBounds(i + 1, j) &&
            this.plan[i][j] === this.plan[i + 1][j])
        ) {
          return;
        }
        highestTile =
          this.plan[i][j] > highestTile ? this.plan[i][j] : highestTile;
      }
    }
...
DOMDisplay.prototype.result = function (state, score) {
  ...
  resultButton = document.querySelector(
    state === 0 ? "#try-again-button" : "#keep-going-button"
  );
  resultTile = document.querySelector("#result-tile");
  resultMessage =
    state === 0
      ? lostMessage
      : wonMessages[Math.floor(Math.random() * wonMessages.length)];

  setTimeout(function () {
    // Display the result box for the appropriate state
    that.resultBox.style.display = "block";
    document.querySelector("#result-background").style.backgroundColor =
